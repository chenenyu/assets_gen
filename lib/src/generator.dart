  ...
  for (Asset asset in assets) {
    ...
    if (pubspec.options.omitPathLevels > 0) {
      ...
      if (pathSegments.isNotEmpty) {
        pathSegments = pathSegments.sublist(min(pubspec.options.omitPathLevels, pathSegments.length));
    ...
    if (pubspec.options.codeStyle == 'lowerCamelCase') {
      ...
      for (int i = 0; i < items.length; i++) {
        var item = items[i];
        item = item.substring(0, 1).toUpperCase() + (item.length > 1 ? item.substring(1) : '');
        // (item.length > 1 ? item.substring(1).toLowerCase() : '');
        result += item;
      }
      key = result;
    ...
    if (asset.isPlural) {
      ...
      content.writeln("  static String ${key.startsWith(RegExp(r'[a-zA-Z$]')) ? '' : '\$'}$key$params => '$body';");
      if (pubspec.options.genPackagePath && !asset.path.startsWith('packages/${pubspec.name}')) {
        content.writeln("  static String ${pubspec.name}\$$key$params => 'packages/${pubspec.name}/$body';");
      }
    } else {
      // 如果key不是以字母或$开头，前面加一个$
      content.writeln("  static const String ${key.startsWith(RegExp(r'[a-zA-Z$]')) ? '' : '\$'}$key = '${asset.path}';");
      if (pubspec.options.genPackagePath && !asset.path.startsWith('packages/${pubspec.name}')) {
        content.writeln("  static const String ${pubspec.name}\$$key = 'packages/${pubspec.name}/${asset.path}';");
      }
    }
  }
  content.writeln('}');
